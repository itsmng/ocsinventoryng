...
pid_dir="/tmp"
pidfile="$pid_dir/ocsng_snmpfullsyncrework.pid"
runningpid=""
scriptname="ocsng_snmpfullsyncrework.sh"
logfilename="ocsng_snmpfullsyncrework.log"
options=""
...
cmd="php ocsng_snmpfullsyncrework.php --ocs_server_id=$server_id"
sh -c "$cmd"
  
while [ $cpt -lt $thread_nbr ]; do 
   cpt=$(($cpt+1))
   cmd="php ocsng_snmpfullsyncrework.php $options --ocs_server_id=$server_id --thread_nbr=$thread_nbr --thread_id=$cpt --process_id=$PROCESS_ID"
   sh -c "$cmd"&
