...
if (isset($_SESSION["ocs_importsnmp"]["id"])) {
   if ($count = count($_SESSION['ocs_importsnmp']['id'])) {
      ...
      $action = PluginOcsinventoryngSnmplinkrework::processSnmp($key,
...
if (!isset($_POST["import_ok"])) {
   ...
   if ($display_list) {
      ...
      $values['tolinked'] = 0;
      PluginOcsinventoryngSnmplinkrework::showSnmpDeviceToAdd($values);
   }
   ...
   if (isset($_POST["toimport"]) && (count($_POST['toimport']) > 0)) {
      ...
      foreach ($_POST['toimport'] as $key => $val) {
         if ($val == "on") {
            ...
            $tableId = $split[0];
            $valueId = $split[1];
            //$_SESSION["ocs_importsnmp"][$tableId][] = $valueId;
