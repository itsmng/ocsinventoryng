...
if (isset($_POST["delete"])
   ...
   if ($count = count($_POST["toupdate"])) {
      ...
      foreach ($_POST['toupdate'] as $key => $val) {
         if ($val == "on") {
            $link = PluginOcsinventoryngSnmplinkRework::deleteSnmp($key,
...
if (isset($_SESSION["ocs_updatesnmp"]['id'])) {
   if ($count = count($_SESSION["ocs_updatesnmp"]['id'])) {
      ...
      $action = PluginOcsinventoryngSnmplinkRework::updateSnmp($key,
         $_SESSION["plugin_ocsinventoryng_ocsservers_id"]);
...
if (!isset($_POST["update_ok"])) {
   ...
   if ($display_list) {
      PluginOcsinventoryngSnmplinkRework::showSnmpDeviceToUpdate($_SESSION["plugin_ocsinventoryng_ocsservers_id"],
